...
class OpeningStockScreen extends StatelessWidget {
  const OpeningStockScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return const _StockCountScreen(mode: _StockCountMode.opening);
  }
}

class ClosingStockScreen extends StatelessWidget {
  const ClosingStockScreen({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return const _StockCountScreen(mode: _StockCountMode.closing);
  }
}
...
enum _StockCountMode { opening, closing }

class _StockCountScreen extends StatefulWidget {
  final _StockCountMode mode;

  const _StockCountScreen({Key? key, required this.mode}) : super(key: key);
...
class _StockCountScreenState extends State<_StockCountScreen> {
  ...
  bool get _isOpening => widget.mode == _StockCountMode.opening;
  ...
  @override
  Widget build(BuildContext context) {
    final int savedCount = _isOpening ? 12 : 21;
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: Column(
            children: [
              _StockCountHeader(
                  title: _isOpening ? 'Opening Stock' : 'Closing Stock'),
              Expanded(
                child: ListView(
                  padding: const EdgeInsets.fromLTRB(20, 18, 20, 18),
                  children: [
                    _SessionSummaryCard(isOpening: _isOpening),
                    const SizedBox(height: 16),
                    if (_isOpening) const _InfoNotice(),
                    if (_isOpening) const SizedBox(height: 16),
                    const _SearchField(),
        ...
        bottomNavigationBar: _CountBottomBar(
          ...
          onPrimaryPressed:
              _isOpening ? _submitOpeningStock : _openClosingReview,
        ),
      ),
    );
  }
